    ...
    def __init__(self, matrix: List[List[int]]):
        ...
        for i in range(1, len(matrix) + 1):
            for j in range(1, len(matrix[0]) + 1):
                # value from the original matrix corresponding to this prefix cell
                val = matrix[i - 1][j - 1]
                # sum of all elements above the current row (same columns)
                top = self.prefix_sums[i - 1][j]
                # sum of all elements left of the current column (same rows)
                left = self.prefix_sums[i][j - 1]
                # overlap area that was counted in both `top` and `left`
                top_left = self.prefix_sums[i - 1][j - 1]

                # inclusion-exclusion: current cell + top + left - overlap
                self.prefix_sums[i][j] = val + top + left - top_left
